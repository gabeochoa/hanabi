...
- [x] **macOS Spotlight kickoff mechanism** — RESOLVED: shipped a functional `.app` bundle (`make bundle` → `Hanabi.app`, CFBundleIdentifier `com.hanabi.app`, enriched Info.plist [ShortVersion/category/min-sys/copyright], `plutil -lint` clean, LaunchServices-registered, resource resolution already bundle-aware via `../Resources`). With the bundle identity present, wired REAL CoreSpotlight indexing (`native_spotlight_index` → `CSSearchableIndex`), donating each thread (uniqueId=thread id, title, domain "threads"). Runtime-gated on `NSBundle.mainBundle.bundleIdentifier != nil` so the bare dev binary stays a safe no-op. PROVEN both ways in a windowed run: bundle logs "Spotlight index OK (first item donated to CSSearchableIndex)" (async completion, no error); bare binary logs the no-op. **Deep-link-back DONE**: the indexed item's `contentURL` is `hanabi://thread/<id>`; the bundle declares the `hanabi` scheme (Info.plist `CFBundleURLTypes`) and installs an Apple-event handler (`kInternetEventClass`/`kAEGetURL`, `native_openurl_install`) that parses the id into a mutex-guarded slot; the frame loop drains it (`native_take_open_thread`) into `requestOpenTab` (same seam a sidebar-row click uses) + activates the app. PROVEN end-to-end: with the bundle running, `open "hanabi://thread/t5"` → LaunchServices routes it → app logs `hanabi://thread open -> id=t5` and renders thread t5. **Spotlight = fully complete** (index + deep-link).
