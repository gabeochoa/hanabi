...
- [ ] **D. Tool calls visible in the transcript on REAL data** — real API has NO `Role::Tool` msg; tool calls are `tool_call`/`tool_result` blocks INSIDE assistant messages (parser currently discards them). Add Message fields (name/node/command/output/status/duration) and make the tool-row renderer PREFER real fields over the current hash-derived fakes.
- [ ] **E. Live SSE thread updates** — continuously receive new messages on the open thread (and list) in real time, not just on manual refresh. SSE to `GET /api/v1/sessions/{id}/events`; on a transcript-changing event → debounced refetch newest-N; ignore `connected` + `context_usage`. Extends existing streaming-reply SSE to live updates. `wt/live-sse` — RUNNING.
