...
### #64 — Nothing reports the height an element actually came out at, so every scrolling list keeps two copies of its own layout
...
### #65 — A wrapped label cannot size itself to its text, and it insets that text by an amount only the renderer knows
...
### #66 — An entity created this frame cannot be found by id until the frame merges, so state has to be threaded through the creation call
