...
`first ink` is confirmed after `graphics::end_frame()` by reading the headless Metal render target through `metal_capture_render_texture_to_memory`. The probe converts RGBA to luma and compares the committed frame with the frame presented immediately before it, so the count is the paint that frame made rather than everything that has changed since the event; the pre-event baseline is still captured and is what `ink_before` reports. Each frame estimates its own background from modal luma, then classifies changed pixels as newly gained ink or removed ink. Appearance, text-appearance, and focus-gain outcomes require at least 40 gained pixels; disappearance, text-disappearance, and focus-loss outcomes require at least 40 lost pixels. The 40-pixel floor is above the composer's source-derived 2x16.8 caret (about 34 pixels): `main_pane_system.h:701,7003` supplies the 21-pixel line height and Afterhours `text_area.h:323,340,350` applies 80% height and 2-pixel width. A caret therefore cannot qualify by itself. A flat frame, failed readback, an unread preceding frame, mismatched shape, missing event timestamp, wrong-direction-only change, missing ink, or missing semantic outcome is a failure rather than a number.

The reading is frozen only on a frame that carries qualifying directional ink of its own and whose declared outcome is reached within the bounded render lag of that same frame. Ink and outcome are associated in time rather than merely both having occurred, so an outcome banked on a frame that painted nothing cannot be settled later by unrelated directional ink arriving after the target is already gone. They are associated in space as well: the ink term is what that frame painted, so pixels that changed earlier for reasons that have nothing to do with the named target sit in the comparison basis and satisfy nothing, and a frame that paints nothing new settles nothing however much unrelated paint was banked before it. The basis is the preceding presented frame, so a watch probes every frame it is live for, and a frame whose predecessor went unread is refused rather than measured against a stale one. Key commands account for Afterhours' one-frame synthetic key-delivery boundary without moving `t0`.
