...
- **State model + marks**: `is_attention()` true only for `Attention`;
  `status_glyph()` precedence — a freeze first (nothing the reader does clears
  it, and it can be true of a running thread), then the two asks (waiting on
  you, blocked — one shape, told apart by colour), then a live run, then
  settled review/done, then a pause, and finally idle. One vocabulary for the
  sidebar row and the tab strip both, so the two can never disagree about the
  same thread. Verified on the actual mock seed too, row by row against the
  parity reference.
- **Smart-view filtering**: Blocked = `tag==Blocked || tag==Waiting ||
  tag==Failed` — a thread waiting on an answer and a run that failed both ride
  with blocked, as they do in Puffin — Review = `state==Ready` /
...
`expect_text "…"`, `expect_no_text "…"`, `expect_panes left right focus`,
`reset_clipboard_probe`, `expect_clipboard "…"`, `screenshot path`. A leading
