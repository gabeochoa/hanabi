...
// A dark UI is the case the reference was measured in; the light palette has
// no measured counterpart, so it keeps the token it always used.
inline bool dark_ui() { return theme::window_bg().r < 128; }
// The strip is the WINDOW plane, not a raised one: the empty area right of the
// last tab is continuous with the content below it, and only the active tab is
// a filled shape. (Reference: strip and inactive tab are both the window
// colour; the active tab is the same fill as a selected sidebar row.)
inline afterhours::Color strip_bg() { return theme::window_bg(); }
// Measured off the reference (#2E3A58). This wants to BE the palette's
// selected-row token — the reference uses one colour for both — but that token
// is currently #344880, so the value lives here until the palette agrees.
inline afterhours::Color tab_active() {
    return dark_ui() ? afterhours::Color{46, 58, 88, 255}
                     : theme::selected_bg();
}
