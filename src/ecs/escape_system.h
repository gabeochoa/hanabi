...
struct EscapeSystem : afterhours::System<UIContext<InputAction>> {
    void for_each_with(Entity&, UIContext<InputAction>&, float) override {
        ...
        else if (app->rowMenuOpen || app->messageMenuOpen ||
        ...
        else if (app->nodePopoverOpen)
            app->escape = EscapeIntent::CloseNodePicker;
        else if (app->askFocused && any_text_field_focused())
            app->escape = EscapeIntent::None;
        else if (app->askFocused)
            app->escape = EscapeIntent::DeclineAsk;
        else
