...
$(TEST_DIR)/test_elicitation: tests/unit/test_elicitation.cpp src/api/elicitation.h src/api/types.h $(TEST_HDRS) | $(TEST_DIR)
...
# The compaction divider's labels: durations, token counts, and which parts
# of the running sentence appear when the wire has carried them.
$(TEST_DIR)/test_compaction: tests/unit/test_compaction.cpp src/api/compaction.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_compaction..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_compaction.cpp -o $@
...
$(TEST_DIR)/test_folder_state: tests/unit/test_folder_state.cpp src/ecs/folder_state.h src/ecs/sidebar_buckets.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_folder_state..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_folder_state.cpp -o $@
...
$(TEST_DIR)/test_reply_quote: tests/unit/test_reply_quote.cpp src/ui/reply_quote.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_reply_quote..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_reply_quote.cpp -o $@
...
$(TEST_DIR)/test_ask_card: tests/unit/test_ask_card.cpp src/ecs/ask_card.h src/api/elicitation.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_ask_card..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_ask_card.cpp -o $@

$(TEST_DIR)/test_tool_kinds: tests/unit/test_tool_kinds.cpp src/api/tool_kinds.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_tool_kinds..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_tool_kinds.cpp -o $@

$(TEST_DIR)/test_transcript_reconcile: tests/unit/test_transcript_reconcile.cpp src/ecs/transcript_reconcile.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_transcript_reconcile..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_transcript_reconcile.cpp -o $@
...
$(TEST_DIR)/test_new_thread: tests/unit/test_new_thread.cpp src/ecs/composer_escape.h src/ecs/composer_notice.h src/api/create_outcome.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_new_thread..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_new_thread.cpp -o $@

$(TEST_DIR)/test_focus_routing: tests/unit/test_focus_routing.cpp src/ecs/focus_routing.h $(TEST_HDRS) | $(TEST_DIR)
	@echo "Compiling test_focus_routing..."
	$(CXX) $(TEST_CXXFLAGS) $(TEST_INCLUDES) tests/unit/test_focus_routing.cpp -o $@
...
UNIT_TEST_EXES := $(TEST_DIR)/test_native_extras $(TEST_DIR)/test_menubar $(TEST_DIR)/test_shortcuts $(TEST_DIR)/test_spotlight_catalog $(TEST_DIR)/test_api $(TEST_DIR)/test_auth $(TEST_DIR)/test_send $(TEST_DIR)/test_stream $(TEST_DIR)/test_tools $(TEST_DIR)/test_textinput $(TEST_DIR)/test_input_pipeline $(TEST_DIR)/test_data $(TEST_DIR)/test_settings $(TEST_DIR)/test_agentcloud $(TEST_DIR)/test_elicitation $(TEST_DIR)/test_ask_card $(TEST_DIR)/test_notify_events $(TEST_DIR)/test_find_nav $(TEST_DIR)/test_session_index $(TEST_DIR)/test_subagent_parent_index $(TEST_DIR)/test_sidebar_buckets $(TEST_DIR)/test_folder_state $(TEST_DIR)/test_home_buckets $(TEST_DIR)/test_contains_lower $(TEST_DIR)/test_snippet_text $(TEST_DIR)/test_diff $(TEST_DIR)/test_ellipsize $(TEST_DIR)/test_compaction $(TEST_DIR)/test_trend $(TEST_DIR)/test_tab_colors $(TEST_DIR)/test_footer_geometry $(TEST_DIR)/test_secondary_surface $(TEST_DIR)/test_pane_memory $(TEST_DIR)/test_transcript_reconcile $(TEST_DIR)/test_tool_kinds $(TEST_DIR)/test_transcript_item_index $(TEST_DIR)/test_wrap_count $(TEST_DIR)/test_md_spans $(TEST_DIR)/test_text_cache $(TEST_DIR)/test_widget_retire $(TEST_DIR)/test_gpu_mem $(TEST_DIR)/test_texture_budget $(TEST_DIR)/test_downscale $(TEST_DIR)/test_digest_layout $(TEST_DIR)/test_heap_walk $(TEST_DIR)/test_minimap_scrub $(TEST_DIR)/test_minimap_marks $(TEST_DIR)/test_focus_ring $(TEST_DIR)/test_outbox $(TEST_DIR)/test_atlas_guard $(TEST_DIR)/test_frame_activity $(TEST_DIR)/test_latency $(TEST_DIR)/test_follow_latch $(TEST_DIR)/test_new_thread $(TEST_DIR)/test_focus_routing $(TEST_DIR)/test_theme_contrast $(TEST_DIR)/test_a11y_bridge $(TEST_DIR)/test_anchored_surface $(TEST_DIR)/test_activation $(TEST_DIR)/test_menu_keys $(TEST_DIR)/test_slash_row $(TEST_DIR)/test_tooltip $(TEST_DIR)/test_overlay_lifecycle $(TEST_DIR)/test_control_state $(TEST_DIR)/test_find_memo $(TEST_DIR)/test_div_move $(TEST_DIR)/test_widget_key $(TEST_DIR)/test_settings_catalog $(TEST_DIR)/test_reply_quote $(TEST_DIR)/test_global_hotkeys
